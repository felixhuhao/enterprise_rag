...
class QueryStatsService:
    ...
    async def get_stats(self) -> dict:
        ...
        async with get_db() as db:
            async with db.execute(
                ...
                "COALESCE(AVG(rerank_avg_score), 0) as avg_rerank, "
            ...
            async with db.execute(
                "SELECT COUNT(*) as cnt FROM query_run_stats "
                "WHERE search_mode LIKE '%fallback%' OR search_mode_hyde LIKE '%fallback%'"
            ) as cursor:
                fallback_row = await cursor.fetchone()

        total = row["total"]
        fallback_count = fallback_row["cnt"]
        return {
            "total_queries": total,
            "avg_rerank_score": round(row["avg_rerank"], 3),
            "avg_result_count": round(row["avg_result_count"], 1),
            "fallback_count": fallback_count,
            "fallback_ratio": round(fallback_count / total, 3) if total > 0 else 0,
        }
    ...
    async def get_trend(self, days: int = 30) -> dict:
        ...
        async with get_db() as db:
            async with db.execute(
                """SELECT DATE(created_at) as date,
                          AVG(rerank_avg_score) as avg_rerank,
        ...
        return {
            ...
            "avg_rerank": [round(row["avg_rerank"], 3) for row in rows],
            "avg_result_count": [round(row["avg_result_count"], 1) for row in rows],
